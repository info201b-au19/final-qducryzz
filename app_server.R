...
server <- function(input, output, session){
  ...
  output$data_one_scatter <- renderPlotly({
    ...
    plot_ly(CO2_emission_sum, x = ~GDP_ave, y = ~CO2_emission_ave,
            color = ~Entity, text = ~paste0("Country: ", Entity, "<br>Average CO2 Emission: ", round(CO2_emission_ave, digits = 2),
                                         "T<br>Average GDP: ", round(GDP_ave, digits = 2), "$<br>Average Population: ", round(population)),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "GDP (USD)"), yaxis = list(title = "CO2 Emission (Tons)"), title = "CO2 Emission v.s GDP")
  })
  ...
  output$data_one_CO2_line <- renderPlotly({
    ...
    plot_ly(CO2_emission_nw, x = ~Year, y = ~Emission, type = "scatter", mode = "lines", text = ~paste0('Year: ', Year,
                                                                                                        "<br>CO2 Emission: ", round(Emission, digits = 2)),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "Year"), yaxis = list(title = "CO2 Emission (Tons)"), title = paste0(input$data_page_one_country," CO2 emission"))
  })
  ...
  output$data_one_GDP_line <- renderPlotly({
    CO2_emission_nw <- CO2_emission %>%
      filter(Entity == input$data_page_one_country)
    plot_ly(CO2_emission_nw, x = ~Year, y = ~GDP_per_capita, type = "scatter", mode = "lines", text = ~paste0('Year: ', Year,
                                                                                                              "<br>GDP per capita: ", round(GDP_per_capita, digits = 2)),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "Year"), yaxis = list(title = "GDP per capita (USD)"), title = paste0(input$data_page_one_country," GDP per capita"))
  })
  ...
  output$data_two_scatter <- renderPlotly({
    ...
    plot_ly(HDI_Renewable_sum, x = ~HDI_ave, y = ~Renewable_ave, color = ~Country, text = ~paste0("Country: ", Country,
                                                                                                  "<br>Average HDI: ", round(HDI_ave, digits = 2),
                                                                                                  "<br>Average Renewable Consumption: ", round(Renewable_ave, digits = 2), "%"),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "HDI"), yaxis = list(title = "Renewable Consumption (%)"), title = "Renewable Consumption v.s HDI")
  })
  ...
  output$data_two_HDI_line <- renderPlotly({
    ...
    plot_ly(HDI_Renewable_nw, x = ~Year, y = ~HDI, type = 'scatter', mode = 'lines', text = ~paste0("Year: ", Year, "<br>HDI: ", HDI),
            hoverinfo = 'text') %>%
  ...
  output$data_three_total_emission <- renderPlotly({
    ...
    plot_ly(Emission_Renewable_sum, x = ~Year, y = ~total_emission, type = 'scatter', mode = 'lines', text = ~paste0("Year: ", Year,
                                                                                                                     "<br>Total CO2 Emission: ",
                                                                                                                     total_emission, "T"),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "Year"), yaxis = list(title = "Total CO2 Emission (Tons)"), title = "Global Total CO2 Emission")
  })
  ...
  output$data_three_year_emission <- renderPlotly({
    ...
    plot_ly(Emission_Renewable_nw, x = ~Total, y = ~reorder(Country,Total), type = 'bar', orientation = 'h', text = ~paste0("Country: ", Country, 
                                                                                                              "<br>CO2 Emission: ", Total, "T"),
            hoverinfo = 'text') %>%
      layout(xaxis = list(title = "Total CO2 Emission (Tons)"), yaxis = list(title = "Country"), title = paste0("Total CO2 Emission in ", input$data_page_three_year))
  })
  ...
  output$data_three_energy_usage_pie <- renderPlotly({
    ...
    plot_ly(Emission_Renewable_nw, labels = c("Renewable", "Nonrenewable"), values = ~c(Renewable, 100-Renewable), type = 'pie',
            textposition = 'inside',
            textinfo = 'label+percent',
            marker = list(color = c('rgb(0, 164, 0)', 'rgb(0, 18, 0)'))) %>%
      layout(title = paste0(input$data_page_three_country, " Energy Consumption Breakdown"),
  ...
  observe({
    ...
    updateSelectInput(session, "data_page_three_country", choices = choice)
  })
}
